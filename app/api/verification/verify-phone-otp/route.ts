...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify phone OTP via Valify
    const verificationResponse = await valifyService.verifyPhoneOTP(
      otp, 
      transaction_id, 
      appraiser_id
    );
    ...
    if (updateError) {
      console.error('Failed to update OTP session:', updateError);
    }

    // If verification successful, update appraiser phone verification status
    if (verificationResponse.verified) {
      ...
      if (appraiserUpdateError) {
        console.error('Failed to update appraiser phone status:', appraiserUpdateError);
      }
    }
