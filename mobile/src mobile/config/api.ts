...
class ApiClient {
  ...
  // Community API - Generic method for community endpoints
  async communityRequest(method: 'GET' | 'POST' | 'PUT' | 'DELETE', endpoint: string, data?: any): Promise<ApiResponse<any>> {
    try {
      let response;
      const url = `/api/community${endpoint}`;
      ...
      return { 
        success: false, 
        error: error.response?.data?.error || error.message || 'Community API request failed'
      };
    }
  }
}
...
// Get platform-specific localhost
import { Platform } from 'react-native';

const getLocalBaseUrl = () => {
  if (Platform.OS === 'ios') {
    return 'http://localhost:3000';  // iOS simulator can use localhost directly
  } else {
    return 'http://10.0.2.2:3000';  // Android emulator needs 10.0.2.2
  }
};
