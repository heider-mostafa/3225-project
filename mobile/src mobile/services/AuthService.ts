...
// import * as LocalAuthentication from 'expo-local-authentication' // Temporarily disabled - missing Expo dependencies
// import * as SecureStore from 'expo-secure-store' // Temporarily disabled - missing Expo dependencies
...
export interface AuthUser {
  ...
  role?: 'user' | 'admin' | 'super_admin' | 'broker' | 'appraiser' | 'developer' | 'compound_manager' | 'resident_owner' | 'resident_tenant' | 'security_guard'
  created_at?: string
...
class AuthService {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            compound_name: metadata.compound_name,
  ...
  // COMMENTED OUT: Requires Expo dependencies (expo-local-authentication, expo-secure-store)
  ...
  */
  // STUB IMPLEMENTATION: Replace when Expo dependencies are available
  async initializeBiometrics(): Promise<void> {
    console.warn('⚠️ initializeBiometrics: Expo dependencies not available, using stub implementation')
    // Set default config indicating biometrics are not available
    this.biometricConfig = {
      enabled: false,
      type: null,
      fallbackToPin: false
  ...
  */
  // STUB IMPLEMENTATION: Replace when Expo dependencies are available
  async enableBiometricAuth(): Promise<boolean> {
    console.warn('⚠️ enableBiometricAuth: Expo dependencies not available, using stub implementation')
    Alert.alert(
      'البصمة غير متوفرة',
      'المصادقة بالبصمة غير متاحة حالياً. يرجى استخدام البريد الإلكتروني وكلمة المرور.',
      [{ text: 'حسناً' }]
    )
    return false
  }
  ...
  */
  // STUB IMPLEMENTATION: Replace when Expo dependencies are available
  async authenticateWithBiometrics(): Promise<AuthResponse> {
    console.warn('⚠️ authenticateWithBiometrics: Expo dependencies not available, using stub implementation')
    return { 
      success: false, 
      error: 'Biometric authentication not implemented - Expo dependencies required' 
    }
  }
  ...
  */
  // STUB IMPLEMENTATION: Replace when Expo dependencies are available
  async disableBiometricAuth(): Promise<boolean> {
    console.warn('⚠️ disableBiometricAuth: Expo dependencies not available, using stub implementation')
    // Just update the local config since SecureStore is not available
    if (this.biometricConfig) {
      this.biometricConfig.enabled = false
      await AsyncStorage.setItem('biometric_config', JSON.stringify(this.biometricConfig))
    }
